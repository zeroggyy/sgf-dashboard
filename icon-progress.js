(function setupIconProgress() {
  ...
  function stats() {
    ...
    $('icon-stats').innerHTML = configs.map(([stage,label]) => `<button class="icon-stat ${stage === 'returned' ? 'is-alert' : ''}" data-stat-stage="${stage}" type="button"><span>${label}</span><strong>${stage === 'all' ? icons.length : icons.filter(item => item.stage === stage).length}</strong></button>`).join('');
    $('icon-stats').querySelectorAll('[data-stat-stage]').forEach(button => button.addEventListener('click', () => { $('icon-stage-filter').value = button.dataset.statStage === 'all' ? 'all' : STAGE_LABELS[button.dataset.statStage]; applyFilters(); }));
  }
  ...
  function renderGroups(filtered) {
    ...
    $('icon-groups').innerHTML = [...groups.entries()].map(([key, group]) => {
      ...
      const cards = group.map(item => { const raw = item.raw; const imageId = gyazoId(raw['預覽圖連結']); const preview = imageId ? `<img src="https://i.gyazo.com/${esc(imageId)}.jpg" alt="${esc(item.name)}" loading="lazy">` : '<i class="fa-regular fa-image"></i>'; const flags = [item.missing.length ? `待補 ${item.missing.length}` : '', item.locales.length ? `${item.locales.length} 語系` : '', item.stage === 'returned' ? '退回' : ''].filter(Boolean); return `<button class="icon-card ${item.missing.length ? 'has-missing' : ''} ${item.stage === 'returned' ? 'is-returned' : ''}" data-icon-index="${item.index}" type="button"><span class="icon-thumb">${preview}</span><span class="icon-card-copy"><small>${esc(item.id || '待填 Icon ID')}</small><strong>${esc(item.name)}</strong><span>${esc(raw['主要尺寸'] || '尺寸待補')} · ${esc(STAGE_LABELS[item.stage])}</span><span class="icon-flags">${flags.map(flag => `<em>${esc(flag)}</em>`).join('')}</span></span></button>`; }).join('');
      return `<section class="icon-group ${open ? 'open' : ''}" data-group="${esc(key)}"><button class="icon-group-toggle" type="button"><span><strong>${esc(type)}</strong><small> · ${esc(subtype)}</small></span><small>${group.length} 項</small><i class="fa-solid fa-chevron-down"></i></button><div class="icon-group-body"><div class="icon-card-grid">${cards}</div></div></section>`;
    }).join('');
    ...
    $('icon-groups').querySelectorAll('[data-icon-index]').forEach(button => button.addEventListener('click', () => openDetail(icons.find(item => item.index === Number(button.dataset.iconIndex)))));
  }
