  ...
  function renderDetailEditor(item) {
    ...
    const dateInput = (label, key, mode = 'date', value = data[key] || '') => {
      ...
      const normalized = match
        ? `${match[1]}-${String(Number(match[2])).padStart(2, '0')}${mode === 'date' && match[3] ? `-${String(Number(match[3])).padStart(2, '0')}` : ''}`
        : '';
      const year = Number(match?.[1]);
      const month = Number(match?.[2]);
      const day = Number(match?.[3]);
      const calendarDate = mode === 'date' && match?.[3] ? new Date(year, month - 1, day) : null;
      const validRange = month >= 1 && month <= 12 && (mode === 'month' || (calendarDate && calendarDate.getFullYear() === year && calendarDate.getMonth() === month - 1 && calendarDate.getDate() === day));
      const valid = Boolean(match && validRange && (mode === 'month' || match[3]));
      const invalidNote = rawValue && !valid ? `<small class="theme2-edit-invalid"><i class="fa-solid fa-triangle-exclamation"></i> 原值「${escapeHtml(rawValue)}」不是有效${mode === 'month' ? '月份' : '日期'}，請重新選擇。</small>` : '';
      return `<label class="theme2-edit-field"><span>${label}</span><input type="${mode}" name="${escapeHtml(key)}" value="${valid ? normalized : ''}" min="2020-01${mode === 'date' ? '-01' : ''}" max="2100-12${mode === 'date' ? '-31' : ''}" autocomplete="off">${invalidNote}</label>`;
    };
    ...
    detail.querySelector('#theme2-edit-form')?.addEventListener('submit', async event => {
      ...
      ['企劃開表日', '美術可用交付日', '最終確認日', '退回日期', '重新確認日期'].forEach(key => {
