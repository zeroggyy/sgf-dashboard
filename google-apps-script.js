  ...
  try {
    ...
    for (let i = 2; i < data.length; i++) {
      ...
      tasks.push({
        ...
        group: currentGroup,
        isDone: row[doneIdx] === true || row[doneIdx] === "TRUE",
...
function doPost(e) {
  try {
    ...
    // 處理試算表自動排序行為
    if (postData.action === "sortTasks") {
      try {
        manualSortTasks();
        return jsonResponse({ success: true, message: "試算表自動排序已完成！" });
      } catch (err) {
        return jsonResponse({ error: "排序函數(manualSortTasks)執行失敗: " + err.toString() }, 500);
      }
    }
