...
function setupEventListeners() {
  ...
  if (scheduleBtn && scheduleModal && closeScheduleBtn) {
    scheduleBtn.addEventListener('click', () => {
      ...
      scheduleYearSelect.value = curYear.toString();
      scheduleMonthSelect.value = curMonth;
      
      renderMilestones(curYear.toString(), curMonth);
    ...
    scheduleYearSelect.addEventListener('change', () => {
      renderMilestones(scheduleYearSelect.value, scheduleMonthSelect.value);
    });
  ...
  statusChips.querySelectorAll('.chip').forEach(chip => {
    chip.addEventListener('click', (e) => {
      if (e.target.classList.contains('disabled')) return; // 若被壓灰則禁止點擊
  ...
  specialChips.querySelectorAll('.chip').forEach(chip => {
    chip.addEventListener('click', (e) => {
      ...
      // 連動時間區間篩選器
      const nonAllTimeChips = timeChips.querySelectorAll('.chip:not([data-time="all"])');
      const allTimeChip = timeChips.querySelector('.chip[data-time="all"]');

      // 連動任務狀態篩選器
      const nonAllStatusChips = statusChips.querySelectorAll('.chip:not([data-status="all"])');
      const allStatusChip = statusChips.querySelector('.chip[data-status="all"]');
...
async function handleFormSubmit(e) {
  ...
  const weeksPayload = {}; // 用於向 API 發送多個週進度變更

  // 1. 收集置頂「上週、當週、下週」輸入框數值並覆蓋 (若有綁定 data-week)
  ...
  timelineInputs.forEach(input => {
    ...
    if (weekLabel) {
      ...
      updatedWeeks[weekLabel] = val;
      weeksPayload[weekLabel] = val;
    }
  });

  // 收集 Checkbox 完成標記 (對應 A 欄) 與 超連結 (對應 B 欄)
  const isDone = editIsDone.checked;
  const taskLink = editTaskLink.value.trim();
  const taskId = editTaskId.value.trim(); // 收集 ID 欄位變更 (B 欄號碼)
  const owner = editOwnerSelect.value;   // 收集負責人變更 (D 欄)
  const taskName = editTaskName.value.trim(); // 收集專案項目名稱 (C 欄)
  ...
  // 3. 送出 POST 到 Google Apps Script 同步
  showToast('正在將變更同步到 Google Sheet...', 'info');
  const payload = {
    action: "updateTask",
    rowNumber: rowNum,
    detail: detail,
    taskId: taskId,     // 傳送專案項目 ID，將由後端寫回 B 欄
    taskName: taskName, // 傳送專案項目名稱，將由後端寫回 C 欄 (保留公式)
    owner: owner,       // 傳送負責人，將由後端寫回 D 欄 (原主)
    isDone: isDone,     // 傳送已完成狀態，將由後端 Apps Script 獨立寫入 A 欄，不影響 E 欄公式
    taskLink: taskLink, // 傳送專案超連結 URL，將由後端寫為 =HYPERLINK 公式
    weeks: weeksPayload // 傳送整組週更新包
  };

  const success = await syncTaskToGoogleSheet(rowNum, payload);
  if (success) {
    showToast('同步成功！正在更新公式連動後的最新狀態...', 'success');
    loadData(); // 重新讀取以取得試算表公式計算後的最新正確進度
  } else {
    showToast('同步失敗，請檢查 API 設定後重新整理。', 'error');
    loadData(); // 重新讀取確保資料一致性
  }
}
...
// 初始化事件監聽
document.getElementById('footer-nonsense').addEventListener('click', showNonsenseModal);
...
function renderMilestones(year, month) {
  ...
  // 3. 生成 1 到 daysInMonth 的完整日期列表，並對齊 milestones 資料
  const fullMonthList = [];
    ...
    <table style="width: 100% !important; border-collapse: collapse !important; border: none !important; margin: 0 !important; padding: 0 !important; table-layout: fixed !important; text-align: left !important;">
      <tbody>
        ${fullMonthList.map((m, idx) => {
          ...
          return `
            <tr class="milestone-row" data-date="${m.date}" data-day="${m.day}" data-row="${m.rowNumber}" data-target="${m.target.replace(/"/g, '&quot;')}" style="border-bottom: 1px dashed var(--border-color) !important; transition: background-color 0.2s !important; ${itemHighlightBg} cursor: pointer;">
              <!-- 1. 日期 (C欄) -->
              <td style="width: 110px !important; padding: 0.7rem 0.5rem !important; font-size: 0.92rem !important; font-family: 'Playfair Display', Georgia, serif !important; text-align: left !important; vertical-align: top !important; line-height: 1.4 !important; user-select: none !important; box-sizing: border-box !important;">
                <span style="${dateBgStyle}">${m.date}</span>
              </td>
              <!-- 2. 星期 (D欄) -->
              <td style="width: 60px !important; padding: 0.7rem 0.5rem !important; font-size: 0.88rem !important; text-align: left !important; color: var(--text-secondary) !important; vertical-align: top !important; line-height: 1.4 !important; user-select: none !important; box-sizing: border-box !important;">
                ${m.day}
              </td>
              <!-- 3. 目標純文字 (F欄) -->
              <td style="padding: 0.7rem 0.5rem !important; font-size: 0.92rem !important; text-align: left !important; vertical-align: top !important; line-height: 1.4 !important; box-sizing: border-box !important;">
                <div style="display: flex !important; align-items: start !important; justify-content: space-between !important; width: 100% !important; text-align: left !important;">
                  <div class="milestone-text-container" style="flex: 1 !important; text-align: left !important; white-space: pre-wrap !important; word-break: break-word !important; ${textColor}">
                    ${displayTarget}
                  </div>
                  ${editIcon}
                </div>
...
function setupMilestoneEditModalListeners() {
  ...
  const submitMilestoneChange = async () => {
    ...
    if (success) {
      ...
      // 2. 重新渲染大事記列表
      const year = scheduleYearSelect.value;
      const month = scheduleMonthSelect.value;
      renderMilestones(year, month);
