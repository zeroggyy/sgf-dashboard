...
(function setupTheme3WeaponDemo() {
  ...
  function openWeaponStyleEditor(weapon) {
    ...
    container.querySelector('#theme3-save-weapon-style')?.addEventListener('click', async event => {
      ...
      try {
        ...
        showToast('已更新確認總表的音效風格。', 'success');
        renderDetail();
      } catch (error) {
        console.error('Theme 3 weapon style update failed', error);
        showToast(`儲存風格失敗：${error.message}`, 'error');
  ...
  function renderActionEditorTab(weapon, action, type, voiceKey) {
    ...
    if (type === 'voice' && !voiceEntry) { showToast('此動作尚無角色語音資料。', 'error'); return; }
  ...
  document.getElementById('theme3-action-save')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!saves.length) {
        showToast('目前沒有需要儲存的變更。', 'error');
        return;
      }
      closeActionEditor();
      showToast(`正在背景儲存 ${saves.length} 項變更；你可以繼續操作。`, 'success');
      Promise.all(saves).then(async () => {
        ...
        showToast(`已儲存 ${saves.length} 項變更並同步至 Google Sheet`, 'success');
      }).catch(error => {
        console.error('Theme 3 Google Sheet API background update failed', error);
        showToast(`背景儲存失敗：${error.message}`, 'error');
      });
    } catch (error) {
